            ...
            for byte in range(16):
                if known_perm:
                    for byte_perm in range(16):
                        ...
                        self.predictions[byte_perm][batch_size*batch:batch_size*(batch +1)] = tf.add(self.predictions[byte_perm,batch_size*batch:batch_size*(batch +1)], tf.expand_dims( get_hot_encode(self.permutations[batch_size*batch:batch_size*(batch +1),byte],16)[:,byte_perm],1) * predictions_non_permuted[byte,batch_size*batch:batch_size*(batch +1)] ) 
